<template>
  ...
  >
    ...
    <!-- 主信息 - 调课已完成：以 Completed 为主，附加调课详情 -->
    <div v-if="status === 'rescheduled' && isRescheduledCompleted" class="flex min-w-0 flex-1 flex-col gap-1">
      ...
      <!-- 调课详情：原 → 新 -->
      <div class="mt-1 flex items-center gap-2 text-xs text-amber-600 dark:text-amber-400">
        <span class="text-gray-400 line-through dark:text-gray-500">
          {{ overriddenDate || date }} · {{ startTime }} – {{ endTime }}
        </span>
        <i class="i-lucide-arrow-right size-3 shrink-0" />
        <span class="font-medium">
          {{ date }} · {{ overriddenStartTime || startTime }} – {{ overriddenEndTime || endTime }}
        </span>
        <span
    ...
    <!-- 主信息 - 非调课状态 -->
    <div v-else-if="status !== 'rescheduled'" class="flex min-w-0 flex-1 flex-col gap-1">
      <div
